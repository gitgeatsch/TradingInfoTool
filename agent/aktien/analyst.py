...
Konfluenz ODER NUR ein knapp ausreichendes CRV, aber nicht beides) rechtfertigt lediglich \
eine moderate Abwertung, keinen Fall unter 75%. Erst ein GENUIN starkes Gegenargument, bei \
dem MEHRERE dieser Schwachpunkte gleichzeitig zutreffen (z.B. widerspruechliche Konfluenz \
UND ein nur knapp ausreichendes CRV gleichzeitig), darf nicht mit hoher Konfidenz (>75%) \
kombiniert werden. Beispiel: "EMA-Ordnung bearish, aber RSI/MACD bullish UND CRV nur bei \
2,08" bei 80% Konfidenz ist inkonsistent - der DOPPELTE Widerspruch muss die Konfidenz nach \
unten ziehen. Nutze dabei die volle Bandbreite von `confidence_pct` (0-100%), auch Werte \
deutlich ueber 75%, wenn das Gegenargument tatsaechlich schwach ist - eine Konfidenz, die \
praktisch immer im selben schmalen Band landet, ist selbst ein Kalibrierungsfehler.
...
Faktoren zaehlen.
27. Exit-Abwaegung fuer aktiv gehaltene taktische Assets OHNE Regel-6-These \
...
28. Stop-Loss-Abstand (2026-08-02, hergeleitet aus einer mechanischen Basislinie \
...
29. Chance-Risiko-Verhaeltnis, gemessene Einordnung (2026-08-06, neu): der \
...
SCHEMA:
{
  ...
  "eigene_einschaetzung": {"folgen": "ja|nein|mit_vorbehalt", "kurzfazit": "<1 Satz, siehe Regel 26>"}
}"""
