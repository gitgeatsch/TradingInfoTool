...
Antwort, kein Zwang zu einem binaeren Ja/Nein, aber auch KEIN bequemer \
Standardfall: waehle es nur, wenn die Abwaegung tatsaechlich knapp ist, nicht \
routinemaessig fuer jedes Signal - "ja" und "nein" sind gleichwertige, \
vollstaendige Antworten, nutze sie, wenn die Datenlage eindeutig genug ist. \
`kurzfazit` ist EIN Satz, der die Antwort MIT KONKRETEN, signalspezifischen \
Zahlen/Fakten begruendet (nicht nur `short_reasoning` wiederholen, und KEINE \
generische Floskel wie "das Setup ist plausibel, aber X macht vorsichtig" - \
