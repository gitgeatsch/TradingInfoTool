...
(deterministisch, nicht von dir zu berechnen). Bei "VERKAUFEN" gilt dieselbe Pflicht \
GESPIEGELT: Stop-Loss ist PFLICHT und das CRV MUSS mindestens 2.0 betragen, konservativ \
gerechnet ((entry_mitte - take_profit.usd_bis) / (stop_loss.usd_bis - entry_mitte)) - \
deine Take-Profit-Zone muss dafuer VOLLSTAENDIG UNTERHALB und deine Stop-Loss-Zone \
VOLLSTAENDIG OBERHALB der Entry-Zone liegen (Take-Profit = Kursniveau, das die bearische \
These bestaetigt; Stop-Loss = Kursniveau, das sie widerlegt). Reicht diese Rechnung \
nicht, wird die Empfehlung ebenfalls nachtraeglich deterministisch auf HALTEN korrigiert.
...
Regel 10)? `confidence_pct` MUSS das dort formulierte Gegenargument widerspiegeln, aber \
GRADUELL, nicht binaer: JEDES reale Setup hat IRGENDEIN Gegenargument - dessen bloße \
Existenz allein ist KEIN Grund fuer eine pauschale Abwertung. Ein EINZELNER, isolierter \
Schwachpunkt (z.B. NUR widerspruechliche Konfluenz ODER NUR ein knapp ausreichendes CRV, \
aber nicht beides) rechtfertigt lediglich eine moderate Abwertung, keinen Fall unter 75%. \
Erst ein GENUIN starkes Gegenargument, bei dem MEHRERE dieser Schwachpunkte gleichzeitig \
zutreffen (z.B. widerspruechliche Konfluenz UND ein nur knapp ausreichendes CRV \
gleichzeitig), darf nicht mit hoher Konfidenz (>75%) kombiniert werden. Beispiel: \
"EMA-Ordnung bearish, aber RSI/MACD bullish UND CRV nur bei 2,08" bei 80% Konfidenz ist \
inkonsistent - der DOPPELTE Widerspruch muss die Konfidenz nach unten ziehen. Nutze dabei \
die volle Bandbreite von `confidence_pct` (0-100%), auch Werte deutlich ueber 75%, wenn das \
Gegenargument tatsaechlich schwach ist - eine Konfidenz, die praktisch immer im selben \
schmalen Band landet, ist selbst ein Kalibrierungsfehler.
...
22. Ist `these_abgleich` NICHT null, hat der Nutzer fuer die Kategorie dieses Assets \
...
Daten - dieser Empfehlung folgen?" `folgen` ist EXAKT einer von "ja"/"nein"/\
"mit_vorbehalt" - "mit_vorbehalt" ist eine echte, eigenstaendige Antwort, kein \
Zwang zu einem binaeren Ja/Nein, aber auch KEIN bequemer Standardfall: waehle \
es nur, wenn die Abwaegung tatsaechlich knapp ist, nicht routinemaessig fuer \
jedes Signal - "ja" und "nein" sind gleichwertige, vollstaendige Antworten, \
nutze sie, wenn die Datenlage eindeutig genug ist. `kurzfazit` ist EIN Satz, \
der die Antwort MIT KONKRETEN, signalspezifischen Zahlen/Fakten begruendet \
(nicht nur `short_reasoning` wiederholen, und KEINE generische Floskel wie \
"das Setup ist plausibel, aber X macht vorsichtig" - jedes `kurzfazit` muss \
...
26. Chance-Risiko-Verhaeltnis, gemessene Einordnung (2026-08-06, neu): der \
...
SCHEMA:
{
  ...
  "eigene_einschaetzung": {"folgen": "ja|nein|mit_vorbehalt", "kurzfazit": "<1 Satz, siehe Regel 19>"}
}"""
...
def _validate(data: dict) -> dict:
    ...
    for eintrag in top_gruende:
        ...
        # 2026-08-02-Fund: das Schema verlangt hier "positionierung", aber long_reasoning
        # (Feld `long_reasoning.fundamental`, siehe Prompt Regel 6/13 + JSON-Schema oben)
        # nennt exakt dieselbe Begruendungsdimension durchgaengig "fundamental" - das LLM
        # greift verlaesslich zum vertrauteren Wort. Alias statt Ablehnung, da beide
        # Begriffe hier dasselbe meinen (siehe Docstring oben: "positionierung...als Ersatz
        # fuer das fehlende" fundamentale Datenquelle bei Rohstoffen).
