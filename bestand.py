...
BEFUNDE = (
    ...
    Befundlage("2.415-alle", "⚠️⚠️⚠️ DIE ZEITFENSTERFRAGE WIRKT BEI JEDEM "
               "BEITRAG ANDERS - eine Neukalibrierung ist deshalb NICHT "
               "neutral. Nutzerauftrag 13.09.: *,welche Indikatoren sind "
               "betroffen und was bedeutet das fuer das Gesamtsystem'*. "
               "GEMESSEN, alle drei mit derselben Mechanik, H20, Haelften "
               "gleich lang: TURNOVER ganz +5,70 (geordnet) · bis 2022 "
               "+2,77 (nein) · ab 2023 +8,64 (JA) -> WIRD UM DIE HAELFTE "
               "STAERKER. FUNDING ganz +2,52 (nein) · bis 2022 +3,23 (nein) "
               "· ab 2023 +1,94 (nein) -> WIRD SCHWAECHER, und es ist in "
               "KEINEM Fenster geordnet: Fuenftel 1 liegt ueber Fuenftel 0. "
               "SCHNITT ganz +3,23 (nein) · bis 2022 +3,18 (nein) · ab 2023 "
               "+3,30 (JA) -> wird ERSTMALS geordnet, bei gleicher Spanne. "
               "➔ EINE UMSTELLUNG AUF ab-2023 VERSCHIEBT DIE GEWICHTE "
               "ZWISCHEN DEN BEIDEN LIVE-BEITRAEGEN: turnover +52 %%, "
               "funding -23 %%. Das ist eine Aenderung an der Bewertung, "
               "nicht an einer Messung. ⚠️ Und der Nebenbefund ueber "
               "`funding` ist eigenstaendig: es ist in keinem der drei "
               "Fenster monoton - das gehoert geprueft, unabhaengig von "
               "jeder Umstellung", "offen",
    ...
    Befundlage("2.413-zerfall", "⚠️⚠️⚠️ DIE REGISTRIERTE TABELLE IST IM "
               "LETZTEN JAHR NICHT REPRODUZIERBAR - UND ZWAR AUF BEIDEN "
               "BASEN. Gemessen mit gemeinsamem Zeitfenster (Vorgabe: sonst "
               "misst man den Kalender), Horizont 20, dieselbe Mechanik: "
               "ALT ueber 2.654 Tage (+3,20 / +0,91 / +0,18 / -1,79 / -2,50, "
               "Spanne +5,70, sauber geordnet) · ALT ueber die letzten 341 "
               "Tage (+3,86 / -1,89 / -1,70 / -0,81 / +0,55, Spanne +3,31) · "
               "NEU ueber dieselben 341 Tage (+1,28 / -1,25 / -0,03 / -0,42 "
               "/ +0,42, Spanne +0,87). ⚠️ IN BEIDEN KURZEN FASSUNGEN GIBT "
               "ES KEINE ORDNUNG MEHR: Fuenftel 1 ist schlechter als "
               "Fuenftel 4. ➔ DER UNTERSCHIED IST ALSO NICHT DIE "
               "DATENQUELLE - er ist der ZEITRAUM. Die schoene Monotonie "
               "stammt aus sieben Jahren; im letzten Jahr traegt `turnover` "
               "auf KEINER der beiden Basen. ⚠️⚠️ WAS DAS NICHT IST: ein "
               "Widerruf. 341 Tage sind wenig, es ist keine Trennschaerfe "
               "gerechnet und kein Nullmodell gefahren - ein "
               "Fuenftel-Median ueber 341 Tage rauscht. Und 2.163 hat die "
               "Zeitstabilitaet ueber HAELFTEN der Historie gemessen und "
               ",stabil bis 0,10 R' gefunden. ⚠️ WAS ES IST: ein Hinweis, "
               "der VOR jeder Umstellung geklaert gehoert - und der einen "
               "der zwei tragenden Beitraege betrifft", "offen",
               "Messung 13.09. mit gemeinsamem Zeitfenster; Befunde 2.163, "
               "2.410"),
    Befundlage("2.413-dilemma", "⚠️⚠️ DAS DILEMMA, DAS TEIL 3 ZU ENTSCHEIDEN "
               "HAT - und es ist keine Kleinigkeit. Die beiden Basen sind "
               "nicht ,alt gegen neu', sondern jede hat einen Mangel, den "
               "die andere nicht hat: ALTE BASIS - sieben Jahre Historie "
               "(2.654 Tage), aber die FALSCHE Menge (Gesamtausgabe statt "
               "umlaufend, bei 16 von 33 Symbolen um 5 bis 74 %% daneben) "
               "und nur 66 Symbole; ausserdem Stueckvolumen EINER Boerse. "
               "NEUE BASIS - die richtige Groesse aus EINER Quelle, 246 "
               "Symbole, USD-Volumen ALLER Boersen, aber nur 365 Tage: mehr "
               "gibt CoinGecko ohne Schluessel nicht her (`days=max` -> HTTP "
               "401). ➔ EINE UMSTELLUNG WUERDE DIE TABELLE AUF EINEM "
               "SIEBTEL DER HISTORIE NEU KALIBRIEREN. Das ist nicht "
               "automatisch besser - es tauscht einen Fehler in der GROESSE "
               "gegen einen Mangel an DATEN. ⚠️ Und im kurzen Fenster traegt "
               "keine von beiden (2.413-zerfall), also entscheidet die "
               "Messung die Frage derzeit nicht", "offen",
    ...
    Befundlage("2.412-lage", "⚠️⚠️ DER REGISTRIERTE STAND VON `turnover` IST "
               ...
               "oder endgueltig fallen. Beides ist ein Ergebnis", "offen",
    ...
    Befundlage("2.411-import", "⚠️ ZUM ZWEITEN MAL AN EINEM TAG: "
               ...
               "hole_fremdreihen.py"),
    Befundlage("2.410", "⚠️⚠️⚠️ DER GEMESSENE UND DER ANGEWENDETE "
               "`turnover` SIND NICHT DIESELBE GROESSE - bei der HAELFTE "
               "der Symbole. Gefunden am 12.09. beim Suchen nach einer "
               "P-Scan-Datenbasis, betrifft aber den LAUFENDEN BETRIEB und "
               "einen der ZWEI tragenden Beitraege. DIE FORMEL IST BEIDE "
               "MALE `Volumen / (Preis x Umlaufmenge)` - aber die "
               "UMLAUFMENGE kommt aus zwei verschiedenen Quellen: "
               "`rechne_turnover_beitrag.py` nimmt `splycur` aus "
               "`data/onchain_historie.db` (66 Symbole), "
               "`marktrang.turnover_werte()` nimmt `circulating_supply` von "
               "CoinGecko. GEMESSEN an 33 vergleichbaren Symbolen: 17 "
               "weichen unter 5 %% ab, ⚠️ 16 (48 %%) ab 5 %% - und teils "
               "massiv: GNO -73,6 %%, XLM -66,7 %%, DOT +45,7 %%, QNT -40,5 "
               "%%, UNI -37,7 %%, XRP -37,1 %%, BNB -29,3 %%, LINK -25,2 %%. "
               "⚠️⚠️ LINK STEHT IN DER WATCHLIST. DIE URSACHE IST SICHTBAR: "
               "die Onchain-Werte sind runde Zahlen (UNI 1.000.000.000, LINK "
               "1.000.000.000, GNO 10.000.000) - das ist die "
               "GESAMTAUSGABE, nicht die UMLAUFENDE Menge. Zwei "
               "verschiedene Begriffe unter einem Namen. ➔ DAS IST DER "
               "H-FEHLER IN REINFORM (,die Anwendung reicht weiter als die "
               "Messung'), nur schlimmer: sie reicht nicht weiter, sie "
               "misst etwas anderes. Ein Beitrag, dessen Kennzahl im "
               "Betrieb um 25 bis 74 %% von der gemessenen abweicht, ist "
               "nicht der gemessene Beitrag", "gilt",
               "data/onchain_historie.db; agent/marktrang.turnover_werte; "
               "CoinGecko-Markets 12.09."),
    Befundlage("2.410-loesung", "➔ DIE LOESUNG LOEST BEIDE PROBLEME AUF "
               "EINMAL - und sie braucht keine neue Quelle. `turnover = "
               "Volumen / (Preis x Menge)` kuerzt sich zu `Volumen / "
               "MARKTKAPITALISIERUNG` - und beide Groessen liefert CoinGecko "
               "`market_chart` HISTORISCH und KOSTENFREI. Praktisch geprueft "
               "am 12.09.: `days=365` gibt 366 Tage mit `prices`, "
               "`market_caps` UND `total_volumes`, ohne Schluessel; "
               "`days=max` verlangt einen (HTTP 401). Gegengerechnet an ADA: "
               "Historie 0,039040 gegen Live-Funktion 0,039041 - identisch. "
               "✔ WAS DAS BRINGT: (a) Messung und Anwendung nutzen "
               "DIESELBE Quelle und dieselbe Supply-Definition - 2.410 ist "
               "behoben; (b) die Messbasis waechst von 66 auf 233+ Symbole; "
               "(c) kein Sammeln ueber Wochen, sondern ein Abruf je Symbol "
               "(233 Aufrufe, rund acht Minuten bei 30/Minute); (d) die "
               "`onchain_historie.db` wird fuer `turnover` NICHT MEHR "
               "GEBRAUCHT. Auch die FUNDING-Historie ist frei: Binance "
               "`/fapi/v1/fundingRate` liefert ohne Schluessel 500 Saetze = "
               "166 Tage je Abruf, paginierbar. ⚠️⚠️ ABER R-R11 GILT: bevor "
               "auf der neuen Basis gemessen wird, muss der bestehende "
               "Befund ,turnover traegt' auf der ALTEN Basis REPRODUZIERT "
               "werden. Traegt er auf der neuen nicht mehr, ist das ein "
               "Widerruf und kein Messfehler", "offen",
    ...
    Befundlage("2.408-vorbehalt", "⚠️⚠️ WAS 2.408 NICHT AUFHEBT: der Beitrag "
               "ist auf 66 Symbolen GEMESSEN und wuerde auf 233 ANGEWENDET. "
               "Das ist die Form des H-Fehlers (*,die Anwendung reicht "
               "weiter als die Messung'*), und sie gehoert benannt. ⚠️ ABER "
               "SIE IST NICHT NEU UND NICHT DURCH DEN P-SCAN ENTSTANDEN: die "
               "Kette bildet den Rang seit dem 31.08. ausdruecklich ueber "
               "den GANZEN Markt, nicht ueber die Watchlist - mit der "
               "Begruendung, dass ein Rang ueber 43 Werte eine ANDERE "
               "Groesse ist als der gemessene (das war der Fehler, an dem H "
               "gescheitert war). Der P-Scan wuerde also nichts Neues tun, "
               "sondern dieselbe Bewertung auf Symbole anwenden, die nicht "
               "auf der Watchlist stehen. ➔ Die Frage ,traegt der Beitrag "
               "auch ausserhalb der 66 gemessenen Symbole' bleibt offen - "
               "sie ist der eigentliche Inhalt von Stufe 2 des P-Scan "
               "(SCHATTEN messen, bevor empfohlen wird)", "offen",
    ...
    Befundlage("2.396-e2e", "✔ DER E2E-NACHWEIS SCHLAEGT FEHL - UND ES LIEGT "
               ...
               "eine frische Sicherung wiederholt", "offen",
    ...
    Befundlage("2.390-warnungen", "⚠️⚠️ DIE MAIL WARNT MEHR ALS SIE SAGT "
               ...
               "der Stopabstand 8,9 %% dreimal", "offen",
    ...
    Befundlage("2.387-fortschreibung", "⚠️ OFFEN NACH DEM ROLLOUT: SECHS "
               ...
               "OD7H, OD7L, OD7N, CEBS, DBPK, EXH3, VVMX) mit 3 bis 4 Tagen "
               "Fortschreibung - genau die, die yfinance am 02.09. 31-mal als "
               ",possibly delisted' meldete und die damals als FREMDE Quelle "
               "abgehakt wurden. ✔ Die Fortschreibung ist gedeckelt "
               "(`MAX_FORTSCHREIBEN_TAGE = 4`), danach zaehlt der Wert als "
               "ohne Kurs und drueckt die Abdeckung - sie kann also nicht "
               "unbemerkt weiterlaufen. ⚠️ Aber: eine Meldung aus fremder "
               "Quelle hat Folgen in der eigenen Rechnung, denn aus dem "
               "Kapital kommt der Hebel-Deckel. Zu pruefen sind die TICKER "
               "(VVMX.DE, DBPK.DE, X136.MU, EXH3.DE, CEBS.DE, ISOC.SG), nicht "
    ...
    Befundlage("2.380-annahmen", "⚠️⚠️ ZUR ABSTIMMUNG - drei gesetzte "
               ...
               "welcher Stop bei der Boerse liegt", "offen",
