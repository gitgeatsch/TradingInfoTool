...
BEFUNDE = (
    ...
    Befundlage('2.455-mail-verloren', '⚠️⚠️ EINE EMPFEHLUNGSMAIL GING VERLOREN, UND NIEMAND MERKTE ES (Export 16.09., gefunden bei der Pruefung von K10). Um 06:51:20 scheiterte der Versand beim Verbindungsaufbau (`smtplib starttls`, in denselben Minuten Zeitueberschreitungen bei Binance - kurze Netzstoerung). Betroffen ist die Rohstoff-Kette 06:51: offene Empfehlungen REDUZIEREN fuer OD7N (219,52 EUR) und OD7H (200,89 EUR). Die Kettenzeile meldet trotzdem ,1 Mails` - `rollen_lauf` zaehlt die ERZEUGTE Mail (`ergebnis[mails].append` vor dem Versand), der Rueckgabewert von `versand()` wird nicht ausgewertet, es gibt keine Wiederholung und keinen Vermerk am Signal. `send_notification_email` loggt nur ERROR. Die ,+6 Job-Fehlschlaege` im Export sind diese Mail, zwei yfinance-Aussetzer bei ROL (14:02) und vier FRED-Ausfaelle (16:00) - KEINER davon kommt vom neuen Bestandsabgleich. ➤ BEHOBEN IM CODE 16.09. (2.455-cash-gebaut, F5, Nutzerentscheidung 1a): ein Wiederholungsversuch nach 60 s, danach ERROR mit Betreff; Vermerk `mail_versand` am Signal; Kettenzeile zaehlt zugestellt und NICHT zugestellt getrennt; Hebelfuehrung vermerkt nur bei Zustellung; Export listet nicht zugestellte Mails - am Notebook K11 offen. Die verlorene Mail vom 16.09. 06:51 wird nicht nachgesendet', 'offen',
