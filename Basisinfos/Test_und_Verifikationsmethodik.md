...
|---|---|---|
| `simuliere_kette.py` | **Laeuft die Abrufkette durch, und kommen die Saetze am Ende an?** Betriebsart `probe` gegen eine KOPIE der Datenbank, echte Reihen/Fakten/Rechnung/Schreiben/Mailaufbau, Attrappe nur fuer die drei Modellaufrufe. Spielt je Instrument JEDE Aktion des Vokabulars durch. **`--nachweis-paket-b`** (Schritt 23, 11.09.2026): ein GEZIELT erzeugtes Hebelgeschaeft ueber die Marktraenge der Attrappe, zwei Kopien (Schalter an/aus) - Signalzeile, Mail, Chart, Hebeltopf, Cooldown, Aggregat-Deckel, Hebelfuehrung mit Position und Plan, Spot unveraendert, Akkumulationssperre. **`--nachweis-n14`**: die OI-Sperre in sechs Faellen. | nach jeder Aenderung an der Rollen-Kette, **vor** jedem Produktivgang; `--nachweis-paket-b` vor dem Rollout von Paket B |
| `ausrollen_paket_b.py` | **Ist am Notebook nach dem Pull alles da, was Paket B braucht?** (Schritt 24, 11.09.2026) Code, config, Schema mit Leseprobe, Kapital frisch, Datenfrische, Hebel-Schalter, Deckel. `--nachrechnen` schreibt die fehlenden Tage in `portfolio_wert_historie` (2.376-rollout); am Desktop nur mit `--db` auf eine Kopie. Getestet gegen die NB-Sicherung 11.09.: 9 Tage, Kapital 9.942 -> 18.213 EUR | beim Rollout am Notebook, vor dem Neustart der App |
...
| `messe_horizont_aus_stop.py` | — | **S2/N-17-0**: sagen Stop-Abstand oder Volatilitätsperzentil die Dauer bis TP/SL voraus? Spearman-Rangkorrelation, Tage-Cluster-Bootstrap (2.109), Terzil-Tabelle. `--selbsttest` gegen zwei Welten. Nullbefund (F-202) |
| `messe_schwelle_h2_kalibrierung.py` | — | **N-17a**: dieselbe Methode wie die ursprüngliche H20-Schwellenkalibrierung, aber mit auf Horizont 2 gebauten Ankern und den H2-Beitragsstufen — Schwelle und Beiträge GEMEINSAM, nicht isoliert (F-203, Warnung aus F-189) |
| `messe_kandidaten_redundanz.py` | — | **N-17b**: sind die tragenden Kandidaten paarweise unabhängig oder messen sie dasselbe zweimal? Spearman je Kalendertag, Median über die Tage, Schwelle \|ρ\|≥0,2. Vorbedingung für jede Kombinationsmessung (F-205) |
...
| `messe_marktscan_wert.py` | — | **Nutzerauftrag 12.09.2026**: hat der Marktscan Wert geliefert? Ertrag je Kandidat gegen die Median-Bewegung der eigenen Symbole in GENAU seinem Fenster (`outcome_gestartet_am` bis `outcome_geprueft_am`), mit Zufallskontrolle ueber 200 Ziehungen. ⚠️ Selektion benannt: gemessen wird nur, was das Tracking verfolgt hat - und das entscheidet der Scan selbst. Selbsttest: Paket `Marktscanwert` |
| `messe_ausstiegsguete.py` | — | **Schritt 48b (12.09.2026)**: war der Verkauf richtig? Kursbewegung nach der Empfehlung gegen die TAGESKLAMMER (Median aller an diesem Tag bewerteten Werte), mit Zufallskontrolle ueber 200 Ziehungen bei gleichen Tagen und zufaelliger Symbolwahl. Horizonte H3/H5/H10/H20. ⚠️ Rechnet mit dem TAGESSCHLUSS, weil der Kurs zum Empfehlungszeitpunkt bei Ausstiegen nicht festgehalten wird - deshalb liegt H3 an der Aufloesungsgrenze. Selbsttest gegen bekannte Wahrheit: Paket `Ausstiegsguete` |
| `messe_haltedauer.py` | — | **Schritt 41 / Befund 2.444-2.445 (13.09.2026)**: wie lange dauert ein Trade wirklich bis zur ersten Marke? Prueft die Mailzahl ,Haltedauer (Zielweg/ATR)^2' gegen gemessene Laufzeiten - Vorabfestlegung im Kopf |
| `messe_stopweite_historisch.py` | — | **Schritt 47 (13.09.2026)**: welche Stopweite traegt? Historisch, gepaart ueber die Messmenge, Band und Trennschaerfe nach Messnorm (Befunde 2.431-2.437) |
| `messe_grundgesamtheit.py` | — | **Befund 2.487-grundgesamtheit (20.09.2026)**: ändert eine andere GRUNDGESAMTHEIT das Fünftel, mit dem die Bewertung rechnet? Derselbe Produktionscode (`marktrang.schnitt_werte`, `_rang`, `_fuenftel`) zweimal, nur mit anderer Messdatei; gezählt wird, bei wie vielen Werten das Fünftel wechselt — über alle gemeinsamen und über die Kettenwerte. ⚠ **Eine Zählung, keine Schätzung**: kein Band, keine Nullwelt, keine Trennschärfe — die Frage lautet ,wie viele`, nicht ,ist es bedeutsam`. Aufruf: `python messe_grundgesamtheit.py <pfad-zur-betriebskopie>`, vier Sekunden |
| `pruefe_stopweite_gegen.py` | — | **Schritt 47, Ehrenrunde**: sieben Gegenproben zu `messe_stopweite_historisch.py` - haengt das Ergebnis an einer frei gewaehlten Annahme? |
| `pruefstand_hebelmail.py` | **vor jeder Aenderung an Mail oder Kette, deren Wirkung man in der Mail sehen muss** | **Schritt 41 (13.09.2026)**: erzeugt die ECHTE Hebelmail aus der ECHTEN Kette im Trockenlauf gegen eine Kopie der Produktionssicherung (im Speicher, nie beschrieben). Echt: Kurse, ATR, Bestand, Kapital, Hebelrechnung, Gliederung; GESTELLT und im Text markiert: Modellantworten und Marktraenge. `--schalter` prueft die Asset-Schalter. LONG und SHORT vergleichen (vier SHORT-Vorzeichenfehler, Schritt 41) |
| `vergleiche_hebel_und_mailrechnung.py` | wenn Gebuehren oder Finanzierung in Frage stehen | **Nutzerauftrag 13.09.2026**: passt die gebuehrenfreie Hebelbewertung (Regel 2) zur Wirtschaftlichkeit in der Mail? Zerlegt die Kosten: die Handelsgebuehr kuerzt sich aus dem Hebel heraus, nur die Finanzierung haengt am Hebel. `--kapital`, `--tage` |
| `pruefe_rollout_14_09.py` | — | **Rollout 14.09.2026, am Notebook, nur lesend**: sind Terminmarkt-Job, Lesegrenze, Umlaufmenge, Migration und Datenfrische angekommen? OK / WARTEN / FEHLER; Volltext nach `Claude_Austauschordner/Pruefungen/` |
...
| `rechne_kandidaten_beitrag.py` | **wenn ein Kandidat auf Frontloading trägt** | **N-17c/F-210**: rechnet Schritt 2 des in 9.6 vorgeschriebenen Verfahrens — die Beitragstabelle je Fünftel in echten R, für alle Frontloading-Kandidaten über beide Hebel-Horizonte. Monotonie als vorab gesetzte Bedingung, `zufall` als mitlaufende Kontrollgröße, und die **Reproduktion von F-203s turnover H2 als Gegenprüfung gegen eine unabhängige Implementierung**. ⚠️ Die Umrechnung in Beitragspunkte setzt eine Wirkung **in R** voraus — eine Frontloading-Wirkung (Anteil 0…1) ist dafür ungeeignet |
| `messe_umkehr_je_turnover.py` | — | **N-17d/F-211**: kippt die Kurzfrist-Umkehr bei hohem Turnover? Faecher je Kalendertag, Wirkung je Fach aus der **echten** `messe_regel_wirksamkeit.wirkung()`, und der **gepaarte** Vergleich der Differenz (2.105) statt zweier Baender nebeneinander. `--selbsttest` gegen zwei Welten mit bekannter Antwort |
| `messe_kombi_gegen_f165_bar.py` | — | **F-208**: erreicht die Kombination F-165s eigene Instrumentwahl-Schwelle (Anteil frontlastig, Punkte-Verschiebung)? Rechnet dieselbe Tabelle wie F-165, über die echte Auswahlfunktion (`_auswahl_je_tag`, `FL.wahl_je_tag`s Top-k-Regel), auf der F-204-bereinigten Basis — samt Nachmessung von F-165s Originalkandidaten, deren Zahlen vor N-19/F-204 lagen. Ergebnis: keine Kombination schlägt `turnover` allein; F-165s eigene Zahlen liegen heute höher als 01.09. |
| `messe_geometrie_invers.py` | — | **Befunde 2.628/2.629 (26.09.2026)**: die Geometrie fuer die INVERSE Achse — Horizont x Stopweite, dann Ausloeser x Abstand. ⭐ Drei Unterschiede zu 2.620: Auswahl je TAG statt globaler Schwelle, Nullwelt TAGESTREU statt gepoolt (daran fiel 2.608), Stopweiten nach UNTEN erweitert (0,15 bis 2,0). ⭐⭐ Berichtet zusaetzlich die STOPQUOTE — eine Zelle, in der der Stop kaum greift, ist als Risikobegrenzung wertlos. ⚠⚠ Hier fielen zwei eigene Fehler auf: die Stopquote ueber die DAUER zu rechnen verpasst den Stop in der letzten Stunde (Sprung 15,9 auf 97,0 Prozent zwischen H2 und H3), und P1 prueft die LOKALE Funktion gegen die bestehenden — eine Probe auf fremdem Code sagt nichts ueber den eigenen |
| `messe_inverse_achse.py` | — | **Befund 2.626 (26.09.2026)**: traegt die INVERSE Achse (niedrigster `ema_abstand_atr`) out of sample und je Asset? ⭐ Die Auswahl laeuft JE TAG (die besten k), nicht ueber eine globale Schwelle — sonst misst man die Tageswahl mit. Zielgroesse ist der Ertrag RELATIV ZUM EIGENEN TAG, Nullwelt tagestreu. ✔ 12 von 12 Zellen, 115 von 115 Symbolen, Positivkontrolle 5/5, und der Gewinn ist ABSOLUT handelbar (+1,33 % gegen -0,05 %). ⚠⚠ Trotzdem fuer den HEBEL kaum brauchbar (Nutzereinordnung): es ist Mean Reversion, der Hebel braucht eine kurze schnelle Aufwaertsbewegung. Gehoert zum Spot- und Akkumulationsarm |
| `messe_endetest_out_of_sample.py` | — | **Befunde 2.624/2.625 (26.09.2026)**: der Ende-zu-Ende-Test vor der Verdrahtung — traegt eine auf Haelfte A KALIBRIERTE Tabelle auf Haelfte B? ⚠⚠ B6 beantwortet das NICHT: es prueft, ob ein Befund in beiden Haelften GILT, nicht ob eine Kalibrierung traegt. ⭐ Rollierend ueber fuenf Fenster, mit LUECKE von H Stunden dazwischen (ein Anker am Fensterende saehe sonst drei Tage in das Prueffenster hinein), und die Geometrie wird MITKALIBRIERT statt uebernommen. ⭐⭐ ZWEI NULLWELTEN, beide ausgewiesen: gepoolt und TAGESTREU (je Tag so viele Anker wie die echte Auswahl). Die Differenz zeigt, wieviel allein die TAGESWAHL traegt — und genau daran fiel 2.608. ⚠ Hier fiel auf, dass eine freie Zufallsziehung die Tagesclusterung zerstoert und das Nullband zu eng macht; `messnorm` haelt das mit *12,4 Punkte auf einem Nulleffekt* fest |
| `messe_hebel_neudimension.py` | — | **Befunde 2.620/2.621 (26.09.2026)**: die Neudimensionierung — Horizont, Stopweite, Trailing-Ausloeser und -Abstand auf EINER Ankermenge. ⭐⭐ Gestaffelt (Stufe G: Horizont x Stop, dann Stufe T: Ausloeser x Abstand), weil 360 Zellen auf 3,18 Mio Ankern nicht rechenbar sind — und Stufe T laeuft auf der besten UND der zweitbesten Geometrie, weil die Staffelung eine Wechselwirkung sonst nicht saehe. Genau die trat ein: bei festem Trailing lag Stop 2,0 vorn, mit optimiertem Stop 2,5. ⚠⚠ Drei Fallen im Bau: (1) `R` ist keine feste Einheit, deshalb Kursprozent daneben; (2) der Auswahlanteil wird angeglichen, nicht die Schwelle; (3) die Hebelhoehe folgt dem GEOMETRISCHEN Ertrag — und der entscheidet hier anders als der Kursertrag (Stop 1,0 gegen 3,0). ⭐ Misst die TATSAECHLICHE Haltedauer je Zelle — als berichteten FAKT fuer Mail und Positionsfuehrung, NICHT als Auswahlkriterium (Regel 2). P1 prueft den Grenzfall gegen `trailing()` aus 2.607 auf Bitgleichheit |
| `messe_coingecko_gegen_binance.py` | — | **Befunde 2.618/2.619 (26.09.2026)**: liefert CoinGecko dieselben Kurse wie Binance? ⚠ Noetig, weil 2.616 an AGGREGIERTEN Binance-Kerzen mass — die Fallback-Stufen waren bis dahin ANNAHMEN. Drei Fragen: Schlusskurse der Preisreihe, High/Low der 4-h-Kerzen, und die daraus gerechnete ATR. ⭐⭐ Der eigentliche Test steht am Ende: BEWERTUNG aus CoinGecko, ERTRAG auf Binance — so waere es im Betrieb. ⚠ Hier fiel auf, dass ein Quellenvergleich DENSELBEN Schaetzer auf beiden Seiten braucht: Renditeschaetzer gegen Spannenschaetzer gab 0,46 und mischte Quelle mit Methode, gleicher Schaetzer gibt 0,9886. ⚠ Und dass ein falscher Zeitschluessel (`%Y-%m-%dT%H` statt `%Y-%m-%d %H:00`) NULL gemeinsame Stunden erzeugt — der Lauf haette leise nichts gemessen. Misst nebenbei, ob die Bewertung auf BTC traegt (aus 9 Werkzeugen ausgeschlossen, im Betrieb freigegeben). Braucht `COINGECKO_API_KEY` |
| `messe_aufloesung_und_fallback.py` | — | **Befunde 2.616/2.617 (26.09.2026)**: haelt die Bewertung eine GROEBERE Aufloesung? Vier Stufen — 1-h-OHLC, 4-h-OHLC (= CoinGecko), Tages-OHLC und ⭐ NUR Schlusskurse ohne High/Low. Die groben Kerzen werden aus den vorhandenen 1-h-Kerzen AGGREGIERT, damit der Unterschied rein die Aufloesung ist und kein Quelleneffekt. ⚠⚠ Zwei Fallen bestimmen den Bau: (1) `R` steht IN ATR — aendert sich die ATR, aendert sich der Nenner, deshalb entscheidet nur der KURSPROZENT-Arm; (2) die ATR wirkt an ZWEI Stellen (Nenner des Merkmals und Stopweite), beide werden gemeinsam UND einzeln umgestellt. ⚠ Hier fiel ein eigener Skalenfehler auf: ohne `sqrt(24/takt)` lag die 4-h-ATR bei 0,42x der 1-h-ATR, und dieselbe Schwelle erzeugte 0,9 statt 18,4 Signale je Tag — deshalb vergleicht das Werkzeug ueber den gleichen AUSWAHLANTEIL, nicht ueber dieselbe Schwelle. ✔ P1 prueft den Grenzfall `takt=1` auf Bitgleichheit — die Selbstprobe steckt im Bau, weil dort derselbe Code laeuft |
| `messe_betrieb_umlegung.py` | — | **Befunde 2.608 bis 2.611 (26.09.2026)**: gilt der Selektionsbefund auch auf der ECHTEN Watchlist? ⭐⭐ Der Nutzer hat hier den Ansatz gekippt — ich hatte ein QUERSCHNITTS-PERZENTIL gebaut (*das schaerfste 1 % je Stunde*), und ein Perzentil ist eine Aussage ueber die MENGE, keine ueber das ASSET: dieselbe Lage hiesse bei 116 Symbolen etwas anderes als bei 28. Das Werkzeug misst deshalb BEIDES — Perzentil und ABSOLUTE Schwelle in ATR — und die absolute ist die betriebstaugliche Form. ⭐ Es prueft ausserdem die ABDECKUNG (welche Watchlist-Symbole haben ueberhaupt Stundenkurse) und die LIQUIDITAET (Medianumsatz je Stunde; keine Bewertung des Assets, sondern eine physische Handelsgrenze). ⚠ Liest `tradinginfotool.db` nur mit `mode=ro` |
| `messe_trailing_und_betrieb.py` | — | **Befund 2.607 (26.09.2026)**: drei Teile in einem Lauf - (1) ABSICHERUNG je KALENDERJAHR (B6 allein zeigt nicht, ob der Effekt im juengsten Fenster noch da ist), (2) die UMLEGUNG auf das System (Signale je Tag, Qualitaet und Hebelstufung je Schaerfe), (3) TRAILING mit nachgezogenem Stop. ⭐ Die Trailing-Rechnung ist an BEKANNTER WAHRHEIT geprueft: monoton steigend/fallend exakt, engere Weite sichert mehr, flacher Kurs null. ⚠ Hier fiel auf, dass `R` in Einheiten des ANFANGSRISIKOS steht - der Vergleich ueber die Trailing-Weiten dreht sich, wenn man absolut in ATR rechnet |
| `messe_ema_laenge.py` | — | **Befund 2.606 (26.09.2026)**, auf Nutzerfrage: welche EMA-Laenge ist der richtige Bezug? Sieben Laengen von 12 bis 1.200 Stunden (bis zum klassischen 50-Tage-EMA). ⭐ Zwei Konstruktionspunkte: der Vorlauf ist fuer ALLE Laengen gleich (sonst verschiedene Grundgesamtheiten), und die Nullwelt zieht SIEBEN Zufallsreihen und nimmt die beste (sonst waere die Wahl der besten Laenge Auslese). ⚠ Hier fiel auf, dass MFE ALLEIN nicht unterscheidet - alle sieben Laengen liegen darueber; erst MFE/MAE trennt |
| `messe_selektion.py` | — | **Befund 2.605 (26.09.2026)**: sind die besten 0,1 Prozent der Lagen wirklich besser? ⭐⭐ Misst die SPITZE statt Fuenftel-Mittelwerte - ein Mittelwert ueber 20 Prozent kann eine Spitze von 1 Prozent nicht zeigen. Zielgroessen MFE (max. Aufwaertsbewegung in ATR), MAE (zugleich die SPIEGELPROBE) und ein Ereignis MIT REIHENFOLGE (Ziel vor Stop). ⭐ Die Nullwelt ist eine ZUFALLSAUSWAHL GLEICHER GROESSE - eine Fuenftelpermutation enthaelt die Verkleinerung der Menge nicht, und eine Auswahl von 0,1 Prozent streut naturgemaess staerker als eine von 20. ⭐ Die DOSIS-WIRKUNG ueber die Schaerfe ist der eigentliche Nachweis - und zugleich die Hebelabstufung |
| `messe_traegt_er.py` | — | **Befund 2.604 (26.09.2026)**: tragen die Richtungsmerkmale aus 2.603 auch `E[R]`? ⭐⭐ Das Fuenftel ist VORHERGESAGT statt ausgewaehlt - damit ist die Messung eine echte Vorhersage und keine Auslese. *Traegt* verlangt drei Dinge zugleich: ueber dem ZUFALLSfuenftel (40 Ziehungen, nicht ueber der Null), von null verschieden bei tagesgeblocktem Fehler, und in BEIDEN Haelften positiv. ⚠ Hier fiel auf, dass das Fuenftel mit dem hoechsten Richtungslift NICHT das mit dem besten `E[R]` ist |
| `pruefe_spiegelprobe.py` | — | **Befund 2.603 (26.09.2026)**: womit trennt man RICHTUNG von BEWEGUNG? ⭐⭐ Geeicht an einer SIMULATION, weil es auf echten Daten keinen garantiert richtungsfreien Testfall gibt - grosse Kryptobewegungen gehen haeufiger nach oben, also steckt ueberall Richtung drin. Fuenf Kandidatengroessen gegen drei Welten (reine Bewegung, reine Richtung, drei Mischungen). ⭐ Die Schwelle wird ABGELESEN (1,717), nicht gesetzt - die registrierte 1,30 liegt unter dem 90. Perzentil der Bewegung-Welt |
| `messe_kandidaten_spiegel.py` | — | **Befund 2.603 (26.09.2026)**: wendet die geeichte Spiegelprobe auf die Kandidaten an - Ereignis ZIEL erreicht, Lift oberstes Fuenftel gegen Mittel, long gegen short. ⚠ Hier wurden die Spiegelproben-Urteile vom 25./26.09. zurueckgezogen: sie waren auf `E[R]` gerechnet, wo Long und Short strukturell gegenlaeufig sind. `zufall` liegt bei 1,019 und belegt, dass die Anlage nicht schief ist |
| `messe_a_faktoren.py` | — | **26.09.2026**: die A-Faktoren aus dem Bauplan vom 25.09. (rsi_umkehr, ema_lage, ema_steigung, ema_abstand_atr, bandenge, trendstruktur) in Kombination A ∧ B ∧ ¬C - mit den bekannten Traegern `momentum_kurz`/`rsi` als A, der Sperre aus 2.602 als ¬C und `vola`/`zufall` als Kontrollen. ⚠ Enthaelt die Auswahl-Nullwelt: wie hoch liegt das BESTE Fuenftel eines ZUFALLSmerkmals? Ohne sie ist *das beste Fuenftel ist positiv* bei 55 betrachteten Werten keine Aussage |
| `messe_sperre_wirkung.py` | — | **Befund 2.602 (26.09.2026)**: was bringt das Wegsperren einer belegten Verlustzelle? Vier Pruefungen - P1 Selbstprobe (arithmetisch gegen gemessen), ⭐⭐ P2 NULLWELT DER UEBERANPASSUNG (dieselbe Suchprozedur auf ZUFALLSachsen: Raster bilden, schlechteste Zelle suchen, sperren - das misst den Anteil, der allein aus dem SUCHEN entsteht; eine zufaellige Sperre gleicher Groesse waere die falsche Nullwelt), ⭐⭐ P3 out-of-sample nach B6 (Sperre auf Haelfte 1 gefunden, auf Haelfte 2 angewandt), P4 Schwellen-Sensitivitaet. ⚠ Hier fiel zum dritten Mal auf, dass kleine Laeufe in die Irre fuehren |
| `messe_nachzieher.py` | — | **Befund 2.601 (25.09.2026)**: welches Asset hat auf BTC noch nicht reagiert? Rueckstand je Asset (roh, beta-bereinigt, gegen Alt-Median), Mitlauf, `beta` und `zufall` als KONTROLLEN. ⭐ Zwei Achsen als Kreuztabelle: BTC-Lage x DOMINANZ-Richtung, mit tagesgeblocktem Fehlerbereich - weil das NIVEAU unter der Ankerclusterung leidet, die ORDNUNG nicht. ⭐⭐ Die Spiegelprobe laeuft ueber `ausgaenge(runter=True)`, also eine echte Short-Geometrie (Stop oben, Ziel unten) - blosses Vorzeichendrehen der Zielgroesse ist tautologisch. ⚠ Hier fiel auf, dass ein Marktzustand MAL BETA innerhalb der Stundenklammer dasselbe Merkmal ist wie Beta allein |
| `messe_regime_rollierend.py` | — | **Befund 2.599 (25.09.2026)**, Schritt 3 des Hebel-Neubaus: ist der MARKTZUSTAND vorab erkennbar? Sechs marktweite Merkmale, streng kausal, gegen `E[R]` BRUTTO (die Niveaufrage). ⭐ Die Nullwelt ist eine ZYKLISCHE VERSCHIEBUNG, nicht eine Permutation - marktweite Merkmale bleiben ueber Tage gleich, eine ankerweise Permutation haette ein absurd enges Band und JEDES Merkmal als Treffer gemeldet. Rueckschau gegen KAUSAL, Fenster als Achse, 10 autokorrelierte Kontrollreihen. ⚠ Urteil an der SPANNE, nicht an der Trefferquote |
| `hole_btc_leitwert.py` | — | **Befund 2.599/2.600 (25.09.2026)**: holt BTC-Stundenkerzen ab 2021-09 in eine EIGENE Datei (`data/btc_leitwert.db`, Marke `_nur_leitwert`). ⚠ Notwendig, weil BTC in `stundenkurse.db` erst ab 2023-09 vorlag - der Leitwert des Marktes hatte die kuerzeste Reihe. Eigene Datei, damit die Ankermengen von 2.597/2.598 unberuehrt bleiben (R-R11). Ergaenzt in BEIDE Richtungen, auch rueckwaerts |
| `pruefe_leitwert_und_regimeform.py` | — | **Fuenf Gegenpruefungen zu 2.599/2.600 (25.09.2026)**: P1 ist der Leitwert bitgleich mit der Messbasis (26.871 Stunden, 0 Abweichungen)? P2 sind die Merkmale streng kausal (am SEITENEFFEKT: Zukunft aendern, Wert bei t muss stehen)? P3 bricht die zyklische Verschiebung den Zusammenhang (0,017 bei 90 Tagen gegen 0,967 bei einem Tag)? P4 sieht das kausale Fuenftel nur die Vergangenheit (Selbstprobe auf monotoner Reihe: 100 %% in Fuenftel 4)? ⭐ P5 fand den Befund 2.600 als NEBENPRODUKT |
| `korrigiere_tageswerte.py` | — | **Altwerkzeug vom 07.08.2026**, am 25.09. beim Erweitern des Werkzeugkasten-Waechters ENTDECKT: es war unverzeichnet, weil das Praefix `korrigiere` in der Waechterliste fehlte. ⭐⭐ Inhaltlich behandelt es GENAU DASSELBE Problem wie Befund 2.600 - der taegliche Job bewertete bis zum 07.08. den LAUFENDEN Tag (Lauf um 06:30, Abdeckung 3 % und 42 % gegen sonst 87-98 %), also die unvollstaendige Periode als abgeschlossene. Behoben durch Bezug auf den Vortag plus eine Abdeckungswache in `agent/portfolio_historie.py`. ⚠⚠ UND DIE LEHRE: derselbe Fehler steckte gleichzeitig in den STUNDENkerzen und blieb dort bis zum 25.09. unentdeckt. Weil dieses Werkzeug nicht im Register stand, fand niemand die Analogie - genau der Zweck, fuer den 2.13 gebaut wurde |
| `korrigiere_letzte_kerze.py` | — | **Befund 2.600 (25.09.2026)**: korrigiert NUR die letzte, offen gespeicherte Stundenkerze je Symbol - absichtlich nicht ueber den Lader, weil der bis zum Ende der Terminmarkt-Spanne holen und damit die Ankermenge verschieben wuerde. Sichert die alten Zeilen, bricht bei verkuerzten Dateien ab (`_nur_symbolliste`, Mindestzeilenzahl - beide Riegel gegen Wegwerfdateien nachgewiesen) |
| `pruefe_rr11_stundenkerze.py` | — | **R-R11 zu 2.600 (25.09.2026)**: halten 2.597 und 2.598 nach der Kerzenkorrektur? ⭐ Die Toleranz ist HERGELEITET, nicht gesetzt: `ausgaenge` sperrt die letzten 24 Anker, also fordert H6/H12 BITGLEICHHEIT und H24 |dE[R]| <= 116/3.237.922 x (CRV+1). Prueft auch die AUSSAGEN, nicht nur die Zahlen. Mit zwei gepflanzten Abweichungen als findend nachgewiesen |
| `messe_drift_herkunft.py` | — | **Befund 2.598 (25.09.2026)**, Schritt 2 des Hebel-Neubaus: woraus besteht die negative Drift aus 2.597 - MARKTFENSTER oder eine TEILMENGE von Symbolen? Drei Achsen (Jahr, Volumen-Fuenftel als kausales 30-Tage-Mittel, Konzentration je Symbol) plus die Weglass-Leiter in ZWEI Fassungen: Rueckschau gegen KAUSAL nach Vorjahresdrift. ⭐ Genau diese Doppelung ist der Kern - die Rueckschau verbessert um Faktor 4,5, die kausale Auswahl verschlechtert monoton. Abnahmeprobe: die gewichtete Summe der Teilmengen muss die Gesamtzahl reproduzieren (R-R11 nach innen) |
| `messe_hebel_geometrie_neutral.py` | — | **Befunde 2.597-* (25.09.2026)**, Schritt 1 des Hebel-Neubaus: ist die Barrierengeometrie LAGENEUTRAL? Stop = k x ATR gegen fixen Prozentstop GEPAART am selben Anker. Zwei Abnahmeproben - (1) Stop in ATR in jeder Lage gleich weit, reproduziert dabei 2.596 im FIX-Arm (R-R11); (2) die Barriere aendert `E[R]` nicht, gemessen gegen die DRIFT statt gegen null (Optional Stopping). Dazu Klammeranteil, Gleichstandszaehler, Nullband (40 Ziehungen Zufallsfuenftel) und Trennschaerfeleiter. ⭐ Hier fiel auf, dass 2.595 `E[R]` gegen den falschen Nullpunkt gemessen hatte |
| `messe_q_beide_seiten.py` | — | **Befund 2.595 (25.09.2026)**, Punkt B: P(Stop vor Ziel) je Lagefuenftel mit DREI Ausgaengen (ZIEL/STOP/OFFEN) und `E[R]` als Hauptgroesse, weil Kelly nur zwei kennt (2.586). Enthaelt den Selbsttest gegen bekannte Wahrheit (300 Nullwelten). ⚠️ Rechnet mit fixem Prozentstop - dieser Mangel ist 2.596, behoben in `messe_hebel_geometrie_neutral.py` |
| `messe_reverse_scharfe_anstiege.py` | — | **Befund 2.594 (25.09.2026)**, Nutzeridee Reverse Engineering: wie ist die LAGE VOR einem scharfen Anstieg? FALL-KONTROLLE statt Mittelwert - Lift = P(Ereignis | oberstes Fuenftel) / P(Ereignis). Mit SPIEGELPROBE (dasselbe Ereignis nach unten, trennt Richtung von Bewegung) und DOSIS-WIRKUNG ueber Vola-Baender |
| `messe_funding_je_geometrie.py` | — | **Befund 2.591 (25.09.2026)**: traegt `funding` auf der Betriebsgeometrie? Mit Bitgleichheitsnachweis gegen `_stop_aus_atr` und einem MECHANISCHEN RIEGEL, der abbricht, wenn `in_r` etwas anderes als 0,0/1,0 enthaelt - die Lehre aus 2.590 (`barriere` zitiert, `bewegung_r` gemessen) |
| `pruefe_barrierengeometrie.py` | — | **Befund 2.591 (25.09.2026)**: vier Barrierengeometrien GEPAART am selben Anker, plus Selbstprobe gegen `_stop_aus_atr` (16/16 bitgleich). Ergebnis: die Messgeometrie stoppt bei 6,18 %, der Betrieb bei 20,59 % |
| `pruefe_symbole_je_termin.py` | — | **Befund 2.592/2.593 (25.09.2026)**: sind 18 Symbole je Termin ein Messstandard oder fehlende Daten? Trichter Stufe fuer Stufe - Antwort: WEDER. funding liefert 116 Symbole/Tag, Terminmarkt 82/Stunde |
...
Vollständig samt Begründung in **Befundkarte 3.9d** (der R-R9-Eintrag).

### ⚠️ Die Schwelle musste mit — nicht aus Systematik, aus Notwendigkeit

`erreichbar_max` fällt auf 0,0489 R. Eine Schwelle von 0,080 hätte für
**jede** Datenlage `traegt_hier = False` ergeben — derselbe Fehler wie am
31.08. („88 % der Werte können nie durch"), für den die Suite seither eine
Prüfung hat.

⚠️ **Das Verfahren wurde vor der Änderung an der alten Lage
REPRODUZIERT** (R-R11): `messe_schwelle_kalibrierung.py` gab dort 0,080
zurück, mit +0,1502 je verworfenem Signal. Erst danach neu gerechnet.

### ⚠️⚠️ Der Befund, der über die Kalibrierung hinausgeht
...
> Tabelle, die nicht existiert. **Die Trennschärfe der Schwelle kam aus
> einer Fiktion.**

Der Durchlass steigt auf mehr als das Dreifache. Das ist keine Lockerung,
sondern das, was übrigbleibt, wenn eine erfundene Unterscheidungskraft
wegfällt.

### ⚠️ Ein Beinahe-Fehler, den das Nachschlagen verhindert hat (R-R10)

Als Gegenprüfung lief `messe_bewertung_kalibrierung.py`: Steigung
**+0,056** Quote-Punkte je R [+0,008 .. +0,098], Kontrolle über zehn
Mischungen sauber (−0,026 .. +0,018) — out-of-sample also **kalibriert der
Richtung nach**, aber nur **16,8 %** der erwarteten +0,333.

**Ich war im Begriff, das als neuen Befund zu führen.** Nachgeschlagen:
das ist **F-215**, und der ist **überholt** — F-218 zeigte, dass jene Form
nicht invariant war, F-219 misst auf invarianter Form **19,5 %**. Das
Werkzeug ist das alte; es reproduziert einen abgelösten Wert.

> **Ein Werkzeug, dessen Befund abgelöst wurde, liefert weiter Zahlen.**
> Vor jeder Deutung gehört geprüft, ob der Befund dahinter noch gilt —
> nicht nur, ob das Skript läuft.

### ✔ Und daraus die Antwort auf eine Nutzerfrage

> *„Warum diese zwei Zahlenwerte — sollten die sich nicht durch die
> Neukalibrierung ergeben?"*

**Der MASSSTAB der Stufen lässt sich herleiten, die FORM nicht.**

- F-219: die Bewertung liefert 19,5 % dessen, was sie behauptet → **alle**
  Stufen sind rund 5× zu groß. Das ist ableitbar.
- Aber es skaliert alle **gleich** und sagt nichts darüber, wie sich
  `turnover`s Fächer **zueinander** verhalten. Genau das konnte N-58 nicht
  messen.
- ⚠️ Und Stufen → `erreichbar_max` → Schwelle ist **sequenziell**: die
  Schwelle wird aus den Stufen berechnet und kann sie nicht bestimmen,
  ohne zirkulär zu werden.
- ⚠️ Das Skalieren ändert Rangfolge und Schwelle **nicht** (beide skalieren
  mit) — wohl aber den **Hebel** (F-220).
...
- `agent/potential.py` — Docstring der Schwelle (er warnt selbst, dass eine
  falsche Zahl dort als Sollwert gelesen wird), `KALIBRIERT_FUER`
- `agent/wahrscheinlichkeit.py` — die Tabelle samt vollständiger Begründung
- `bestand.py` — Kandidateneintrag, drei Befunde
- `messe_bewertung_kalibrierung.py` — fest verdrahtete Altstufen
- Befundkarte **3.9d**
