...
3. Beitrag je Stufe (auswahl, terminmarkt, entscheider) als „mit gegen ohne“. · ⛔ **`auswahl` GEMESSEN 19.09.** (2.479-auswahlstufe): **keine Zelle trägt** — bei H20 ist die Anlage mit Trennschärfe 0,40 R zu grob für ein Urteil, bei H5 löst sie auf (0,05–0,10 R) und die Wirkungen liegen bei −0,008 bis −0,021 R, also darunter. ➔ Die Auswahl ist kein **Qualitätsfilter**, sondern eine **Mengensteuerung** (LLM-Kontingent); konsistent damit, dass `momentum` im Register als Kandidat nicht trägt. ⚠️ Das ist die QUALITÄTSfrage — die WIRKUNGSfrage steht in F-182 (null zusätzliche Werte). ✔ **`entscheider` GEMESSEN 19.09.** (2.480-entscheiderstufe): **+0,2137 R**, Band [+0,1271 .. +0,3182] → **TRÄGT** — der erste klar tragende Stufenbefund. ⚠️ Gegen die **strengere** Nullwelt (nur aus den **Bewertbaren**, +0,0724/oben +0,0919) trägt er weiterhin; der **Datenlagenanteil** ist +0,0461 R, also rund ein Fünftel. **8 von 8 CRV-Werten** tragen. ➔ Die Kette leistet ihre Arbeit an der **Bewertungsstufe**, nicht am Momentum-Sieb. ✔ **`terminmarkt` belegt 19.09.** (2.481-terminmarktstufe): die **Größe** ist registriert und reproduziert; die **Stufe** sperrt nur bei *einstieg* **ohne** Bestand im obersten OI-Fünftel — das sind **4 von 508** Zeilen seit dem 11.09. ⚠️⚠️ Und sie steht **nicht im Trichter** (`PROTOKOLL_AB='urteil'`): null Einträge sind **kein** Beleg für null Wirkung. ➔ **Eine kleine Umsetzung folgt**: `terminmarkt` gehört in `PROTOKOLL_IMMER`. ✔ **Phase 3 Punkt 3 ist damit vollständig**
...
### ⏳ VORGEMERKT — Schritt 64: die Norm nachziehen (A und B, Nutzerentscheidung 19.09.)

⚠️⚠️ **Das Verfahren gibt es im Projekt schon.** `agent/krypto/ausschuss_suche.py` (04.08.) rechnet die Mehrfachtestung seit sechs
Wochen über eine **Max-Statistik statt Einzeltest**, ausdrücklich *ohne Bonferroni und seine
Konservativität* — das ist Romano/Wolf in eigenen Worten. Es ist nur nie in `messnorm` gewandert.

| | Punkt | Ändert Urteile? |
|---|---|---|
| **A** | die Norm rechnet **familienweit** — Maximum über die Zellen aus den **vorhandenen** Nullwelten | ⚠️ **ja**, aber nur bei **Mehrzell**-Fragen. Kein Aufrufer hat je `hypothesen` gesetzt, jeder Befund steht auf 1; wo eine Frage eine Zelle war, hat die Norm gehalten |
| **B** | der Befund trägt seine **Hypothese im Klartext** — Pflichtfeld, **gezogen** aus `bestand.KANDIDATEN` | nein |
| **C** | jede Messung **nennt den registrierten Kandidaten**, dessen Hypothese sie prüft; Stufen per **Verweis** (`auswahl` → `momentum`, `terminmarkt` → `oi_aenderung`) | nein |
| **D** | die **Saatprobe** an der Grenze (2.477-stabilitaet) | ⚠️ ja, bei knappen Urteilen |

⚠️ **Warum C nötig ist:** am 19.09. wurde die Stufe `auswahl` gemessen, **ohne dass die
registrierte Hypothese auftauchte** — sie stand die ganze Zeit im Kandidatenblatt `momentum`
(*wer gestiegen ist, steigt weiter*), und die Auswahl ist nichts anderes als deren Anwendung.

⛔ **Nicht in diesem Paket:** die Begründung im **Betrieb**. In `agent/`, `scheduler/`, `ui/`
kommt `hypothese` nicht vor — die Kette *Hypothese → Begründung* zu schließen gehört zu **Schritt 33**.

⚠️ **R-R11:** A und D können Urteile bewegen — jedes betroffene gehört **zuerst reproduziert**.
**Nicht vor M1**; die laufenden Phase-3-Messungen bleiben deshalb bei **einer Zelle je Frage**,
dann entsteht keine Nacharbeit.
...
| **M1-Kriterium 2, erste Hälfte** | ✔✔ **BEANTWORTET 23.09.: JA** — die Quote bewegt den Hebel. Vorwärts gerechnet (`phase4_traegt_die_quote_den_hebel.py`): die echte Quote spannt **8,4 Prozentpunkte** auf (0,2857–0,3695). Sie entscheidet vor allem das **OB** — je nach Stopweite bleiben **11 bis 25 von 25** Kombinationen Spot; auf die **Höhe** wirkt sie mit Faktor 1,26–2,22, der Stop mit 1,60–1,89 (2.547). ⛔⛔ **ABER DAS IST EINE KONSTRUKTIONS-, KEINE BETRIEBSAUSSAGE.** Die Kennlinie sagt bei 8 % Stop: 14 von 25 Kombinationen ergeben einen Hebel (**56 %**). Im Betrieb sind es **29 von 626 Signalen (4,6 %)** — **Faktor 12** auseinander, weil davor Schwelle, Auswahl und Takt filtern. Sie beantwortet *„wenn ein Signal bis zur Hebelrechnung kommt, welcher Hebel fällt an"* — **nicht, wie oft**. ➔ Offen und **die halbe Antwort**: die Kennlinie mit **echten Ankern gewichten** (welche Kombination wie oft, und wie oft läuft die Kette). ~~NICHT BEANTWORTET~~ (berichtigt 23.09.) | *„Trägt die Quote den Hebel (r(q))?"* — ~~gemessen verneint~~. **Die Messung taugt dafür nicht:** die Quote ist zurückgerechnet aus dem **geklammerten** `r`, also kann sie per Konstruktion nur zwischen 0,340 und 0,350 liegen (Faktor 1,029) — gemessen wurde 1,02. Und 21 der 22 Signale stammen aus der **alten** Kalibrierung. ➔ Was gilt, ist strukturell: solange `r` auf Faktor 2,5 geklammert ist und der Stop frei streut, **kann** die Quote nicht dominieren. Die Klammer schneidet die Wirkung ab — sie fehlt nicht |
...
| **V3a** | ✔ **ERLEDIGT 19.09.** (2.473): `spot × halten` ist als **Messlage** erklärt — mit Guard, der **nie** eine Zelle daraus macht (Halten ist der Ruhezustand, keine Handlung). Gemessen: `funding` **+0,0279 R → TRÄGT**, `oi_aenderung` **+0,0215 R → TRÄGT**, `turnover` nicht trennbar. ⚠️ **Alle Vorzeichen positiv** — anders als bei der Akkumulation (2.287). ✔ **Offene Frage geklärt** (2.474): von 81 Werten stehen 44 noch in der Auswahl, 38 nicht mehr. Auf den 38 — der **eigentlichen Haltefrage** — trägt `funding` weiterhin (+0,0293 R, engeres Band); `oi_aenderung` **nicht mehr** (18 Blöcke, kein Befund). ➔ **V3b darf sich nur auf `funding` stützen** | — | die Verkaufsbewertung darf auf diese Beiträge gestellt werden |
| **V3b-1** | ✔ **GEMESSEN 19.09.** (2.476-verkaufsregel): `funding` als Verkaufsregel — oberstes Fünftel der Haltemenge gilt als verkauft. Ganze Haltemenge **+0,0791 R**, Band [+0,0431 .. +0,1252] → **TRÄGT**; nur noch in der Auswahl +0,0824 R → **TRÄGT**. ⚠️ Auf der **eigentlichen** Haltefrage (38 Werte, heute nicht mehr in der Auswahl) **+0,0178 R, Band [−0,0800 .. +0,0734] → NICHT TRENNBAR** — die Wirkung (+0,1382) liegt weit über der Trennschärfe 0,02 R, das Band deckt sie um **0,0019** nicht. ➔ Die Regel trägt dort, wo der Wert **ohnehin noch kaufenswert** ist | V3a ✔ | die Zielgröße der Verkaufsbewertung ist gemessen |
...
| **N8** | Wiederholungssperre (Uhr, 94 % Filter) gegen Regel 1 | in 59 messen und dann entscheiden / sofort diskutieren | **gemessen 18.09. (2.461-sperre): trägt in keiner Länge** – sie wählt nicht besser aus als der Zufall und bleibt damit allein eine Kostenbremse. ✔ **Uhr je Zelle gebaut 18.09.** (S1, 2.462-uhr-getrennt). ✔ **S3 gemessen 19.09.** (2.464-warten): Warten kostet im Einstiegskurs **nichts** (Median 0,0000 R bis 24 h, Auflösung 0,02 R) – die Sperre verzögert, sie verteuert nicht; der 15-Minuten-Takt ist mit dem Kurs **nicht** begründbar. ⚠️ Die Lücke ist die fehlende **Bewertung**, nicht der Preis (2.464-unterdrückung). ✔ **`anlass`-Schwelle gemessen 19.09.** (2.465-anlass, 2.465-volumen): schärfere Regler treffen **gezielt ruhige Lagen** (10–15 % weniger Bewegung, Urteil RUHIGER gegen Nullwelt), senken das Volumen um 60 % (`mindestens 2`) bis 89 % (`+ ohne marken`). ⚠️ `umschlag` trägt heute 54,9 % der Blockänderungen, nicht `marken` (31,4 %) – die 59,3 % in der config sind überholt. ✔ **Hochrechnung 19.09.** (2.466-hochrechnung): heute erreichen nur **84 Zellen/Tag** das Modell bei 500 Kontingent; ohne Sperre wären es **2.257**, mit `mindestens 2` noch 898, mit `mindestens 3` 189. ⚠️ Die Annahme hält **nicht** (2.466-annahme, Korrelation −0,108) – die Zahlen sind **Untergrenzen**. ⚠️ Dabei gefunden: die Live-Auswahl nimmt **k = 2** und lässt Bestand immer durch, nicht 20 % wie in `phase3_kette` modelliert. ⏳ **Offen:** Nutzerentscheidung über die Einstellung |
