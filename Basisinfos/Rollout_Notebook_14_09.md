...
| `e45aa26` | **Analyseknöpfe für ETF gesperrt** | Die Knöpfe „Signal berechnen“ für Themen-ETF (CEBS, EXH3, G2X, ISOC, VVMX, X136) und Absicherungen (DBPK, 3QSS) sind jetzt gesperrt wie bei Krypto, Aktien und Rohstoffen – vorher hätte ein Klick die alte Pipeline mit echtem Modellaufruf gestartet | 2.456-etf-knopf | ⏳ Pull ausstehend (K13) |
| `03e5724` | **Kennzeichnung nicht vermessener Bereiche** | Kaufmails für Aktien, Rohstoffe, Themen-ETF und Absicherung tragen im Betreff `· Themen-ETF, nicht vermessen` und als erste Zeile `⚠️ NICHT VERMESSEN – …`; Verkaufs-Sammelmails dieser Bereiche nennen den Bereich; Krypto-Mails unverändert; jede neue Signalzeile speichert ihre Gruppe | 2.456-abgrenzung | ✔ 17.09. 18:08 gepullt, Spalte da · ⏳ K14 erste gekennzeichnete Mail |
| `c2e6514` | **Messskripte nur lesend, Messprotokoll** | Acht Mess- und Prüfskripte öffnen die Datenbank nur noch lesend; das Messprotokoll nennt 40 statt 5 Nullziehungen. **Kein Laufzeitcode der App** – `git pull` genügt, **kein Neustart nötig**; am NB nichts zu sehen. Optional: CLAUDE.md-Zeile „Nullpunkt“ (lokale Datei) per USB angleichen | 2.457-nurlesend, 2.457-protokoll | ✔ 17.09. gepullt (mit `65ef6cd` ohnehin enthalten) |
| `65ef6cd` | **Widersprüche der LLM-Rollen registriert, Pläne und Basisdokumente bereinigt** | Nur Dokumentation: neun Widersprüche als Befunde, Landkarte und Regelwerksmanual mit Standvermerk, Planbereinigung. **Kein Laufzeitcode** – `git pull` genügt, kein Neustart, am NB nichts zu sehen | 2.457-w1 … 2.457-w9, 2.457-n2, 2.457-n3 | ✔ 17.09. gepullt |
| `af2d620` | **Marktlage und Zusatzinfo stehen wieder in der Mail** | Jede Kaufmail zeigt unter „Umfeld“ die Marktlage von Rolle A: Gesamtlage, bei Krypto den Gleichlauf, dazu die Einstufung der eigenen Klasse **mit Begründung**. Bei Krypto kommen Kontenanteil und Put-Skew als Zusatzinfo dazu. **Die Frage an das Sprachmodell ist unverändert.** `git pull` **und App neu starten** (Laufzeitcode `agent/rollen_lauf.py`), danach Export | 2.457-w3 | ✔ 18.09. gepullt und neu gestartet · ⏳ K16 (erste Mail mit „Umfeld“) |
| `2f2f22c` | **Die Kette schreibt mehr mit** | Jede Signalzeile trägt jetzt den **Kurs zum Empfehlungszeitpunkt**, das **Potential mit Schwelle** (bisher nur bei abgelehnten Fällen) und die neue Spalte **Phase** (Einstieg / Führung / Ausstieg / Akkumulation). **Nichts davon entscheidet etwas** – es wird nur erfasst. `git pull` **und App neu starten**, danach Export | 2.458-protokoll-1 | ⏳ Pull + Neustart (K17) |
| `9b6cbd5` | **Die Kette schreibt ihre Spur je Zelle** | Neue Tabelle `zellen_lauf`: je geprüftem Wert eine Zeile mit erreichter Stufe, Ergebnis, Grund im Klartext und Verweis auf die Signalzeile – rund 200 Zeilen am Tag. **Nichts davon entscheidet etwas.** `git pull` **und App neu starten** (die Tabelle entsteht beim Start), danach Export | 2.458-protokoll-2 | ⏳ Pull + Neustart (K18) |
| `086dd0b` | **Die Führung wird mitgeschrieben** | Neue Tabelle `fuehrung_lauf`: jede Stop-Nachzieh-Empfehlung **und** jede geprüfte Position ohne Empfehlung – damit ist erstmals messbar, was die Führung leistet (die Mail mit 116 Empfehlungen hinterließ bisher keine Spur). Dazu bekommt die gesperrte Akkumulation eine Zeile am Tag. **Nichts davon entscheidet etwas.** `git pull` **und App neu starten**, danach Export | 2.458-protokoll-3 | ⏳ Pull + Neustart (K19) |
| *(nächster Commit)* | **Ausstiege werden endlich ausgewertet** | Jeder Verkauf und jede Reduktion bekommt nach 5 und 20 Handelstagen eine Zahl: Ist der Kurs danach gefallen (Verkauf richtig) oder gestiegen (Verkauf teuer)? Gilt auch **rückwirkend** für die Altzeilen. **Nichts davon entscheidet etwas** – bewertet wird erst in Phase 5. `git pull` **und App neu starten**, danach Export | 2.458-protokoll-4, 2.401 | ⏳ Pull + Neustart (K20) |
...
**Nach dem Pull von „Kennzeichnung nicht vermessener Bereiche“ (K14):** zusammen mit K13 – ein `git pull`, **App neu starten** (Migration der Spalte `gruppe` läuft still beim ersten Schreiben), nach **30 Minuten** Export.

- die nächste Kaufmail für eine Aktie, einen Themen-ETF, einen Rohstoff oder eine Absicherung hat im Betreff `… · Themen-ETF, nicht vermessen` (bzw. Aktien/Rohstoffe/Absicherung) und direkt unter dem Titel die Zeile `⚠️ NICHT VERMESSEN – …`
- Krypto-Mails sehen **genauso aus wie bisher**
- eine Verkaufs-Sammelmail dieser Bereiche endet im Betreff auf `· Aktien` usw. und beginnt mit `Bereich … – ohne gemessene Bewertung`
- im Export: Signalliste mit Spalte `gruppe` (neue Zeilen befüllt, alte leer), Spaltendrift leer
- ⚠️ eine Krypto-Mail mit `nicht vermessen` wäre ein Befund
