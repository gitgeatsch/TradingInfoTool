...
| `c2e6514` | **Messskripte nur lesend, Messprotokoll** | Acht Mess- und Prüfskripte öffnen die Datenbank nur noch lesend; das Messprotokoll nennt 40 statt 5 Nullziehungen. **Kein Laufzeitcode der App** – `git pull` genügt, **kein Neustart nötig**; am NB nichts zu sehen. Optional: CLAUDE.md-Zeile „Nullpunkt“ (lokale Datei) per USB angleichen | 2.457-nurlesend, 2.457-protokoll | ⏳ Pull |
...
| *(nächster Commit)* | **Marktlage und Zusatzinfo stehen wieder in der Mail** | Jede Kaufmail zeigt unter „Umfeld“ die Marktlage von Rolle A: Gesamtlage, bei Krypto den Gleichlauf, dazu die Einstufung der eigenen Klasse **mit Begründung**. Bei Krypto kommen Kontenanteil und Put-Skew als Zusatzinfo dazu. **Die Frage an das Sprachmodell ist unverändert.** `git pull` **und App neu starten** (Laufzeitcode `agent/rollen_lauf.py`), danach Export | 2.457-w3 | ⏳ Pull + Neustart (K16) |
