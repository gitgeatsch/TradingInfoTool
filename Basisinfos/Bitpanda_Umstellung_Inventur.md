...
## A · SCHREIBEN ODER HOLEN VON BITPANDA — werden ersetzt oder umgestellt (15)
...
| `importer/bitpanda_bestand.py` | **Netz:** Modulebene, _fusion_schluessel, cash_abgleich<br>**Sync:** bestandsabgleich<br>**Bestand schreibt:** _verschwundene, abgleich_neu<br>**Bestand liest:** Modulebene, _verschwundene, abgleich_neu<br>**Cash:** Modulebene, abgleich_neu, cash_abgleich<br>**Stempel:** abgleich_neu, cash_abgleich<br>**hebel_positions:** Modulebene | NEU 16.09. (Stufe 1.2): Bestandsabgleich ueber die neue Schnittstelle - Salden je Wallet aus `asset_balance_after` (Tabelle `bitpanda_wallet_saldo`), Gegenprobe gegen `/portfolio`, Zuordnung ueber ISIN/Symbol/Alias, Signal-Bestaetigung nur bei echtem Verkauf (E10), verschwundene Positionen (E11), Meldungen mit Name/Wert/Status/Aktion (E13); Waehler `bestandsabgleich` (E12) | 1.3 (16.09.): `cash_abgleich` - verfuegbar bleibt in `cash_reserve_fiat_eur`, gesamt/gebunden und Fusion-Orderdetails als Meta; `_protokoll` schreibt jede Aenderung, E10-Schutz und Cash-Lage ins Log. Stufe 3 Hebel; alter Abgleich erst nach bestaetigtem Betrieb entfernen |
...
| `agent/portfolio_historie.py` | **Netz:** Modulebene, bestandsverlauf<br>**Bestand liest:** compute_hedge_wirksamkeit, rekonstruiere_stichtag, schreibe_tageswert<br>**Cash:** _cash_gesamt | Tageswert/Kapital (`schreibe_tageswert`), Rueckrechnung (`rekonstruiere_stichtag`, `bestandsverlauf` aus alter Historie mit Symbol-Overrides VST-US/IS0C/CC), Hedge-Wirksamkeit | 1.3 erledigt: Spalte `cash_eur` = Cash GESAMT (`_cash_gesamt`), Hebel-Kapital unberuehrt (P-5). 1.5 Rueckrechnung aus `asset_balance_after` je Tag (HEIKEL); braucht 1.9 (OD7H/OD7C-Kurse) vorher |
...
| `extract_notebook_diagnose.py` | **Bestand liest:** main<br>**Cash:** Modulebene, _bitpanda_bestand<br>**Stempel:** _bitpanda_bestand<br>**hebel_positions:** _paket_b, main | Export: Bestand, Cash, Hebelpositionen (Paket B); NEU Abschnitt `bitpanda_bestand` (Quelle, Stempel, Salden je Wallet, Katalog, Cash-Lage, Mailversand 3 Tage) | 1.2/1.3 erledigt 16.09. |
