...
| 9 | Materialitätsschwelle EIA-Erdgas-Lagerbestand: 5-Jahres-Saisonvergleich, ±5%-Schwelle | [GEBAUT] | `agent/kategorie_thesen.py::_abgleich_eia_erdgas()` — n_weeks=270, Kalendertag-Toleranz ±4 Tage je Vorjahr (Schaltjahr-sicher), kombiniert mit COT-Erdgas ausschließlich unter der spezifischeren Kategorie `energie:erdgas` (nicht der Energie-Hauptgruppe, die COT-seitig auch Rohöl poolt). 3 synthetische Tests + echter Live-Abruf (heute: 3.056 Bcf vs. 5J-Schnitt 2.871 Bcf, +6,4% → widerspricht "übergewichten") bestanden |
